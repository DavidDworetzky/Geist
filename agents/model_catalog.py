...
PROVIDERS: dict[str, ProviderSpec] = {
    ...
    "zai": ProviderSpec("zai", "Z.AI", "https://api.z.ai/api/paas/v4", "ZAI_API_KEY"),
    ...
    "meta": ProviderSpec("meta", "Meta Model API", "https://api.meta.ai/v1", "MODEL_API_KEY"),
...
MODEL_SPECS: tuple[ModelSpec, ...] = (
    ...
    ModelSpec(
        ...
        "DeepSeek R1 Distill Qwen 7B (Local)",
        "deepseek-distill",
        context_window=131072,
        max_output_tokens=32768,
        supports_reasoning=True,
    ...
    ModelSpec(
        ...
        local=False,
        performance_note=(
            "Hosted through OpenRouter. Geist disables reasoning because OpenAI's "
            "Chat Completions tool calling requires reasoning effort none. Enforce "
            "OpenRouter ZDR routing before sending confidential workloads."
        ),
    ),
    # Heavyweight models are deliberately server-backed.
    ModelSpec(
        "zai-org/GLM-4.7-Flash",
        "GLM 4.7 Flash (Self-hosted)",
        "glm",
...
def _heavyweight_parameter_count(model_id: str) -> float | None:
    ...
    billion_matches = [
        float(value) for value in re.findall(r"(?:^|[-_/])(\d+(?:\.\d+)?)b(?:[-_/]|$)", model_id)
    ]
