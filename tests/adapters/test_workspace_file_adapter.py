...
def test_additional_credentials_are_excluded_from_all_file_tools(tmp_path, path):
    target = tmp_path / path
    target.parent.mkdir(parents=True, exist_ok=True)
    target.write_text("synthetic-test-value", encoding="utf-8")
    adapter = WorkspaceFileAdapter(str(tmp_path))
    assert adapter.list_files() == []
    assert adapter.search_text("synthetic-test-value") == []
    with pytest.raises(ValueError, match="credential"):
        adapter.read_file(path)
    with pytest.raises(ValueError, match="credential"):
        adapter.write_file(path, "replacement")
