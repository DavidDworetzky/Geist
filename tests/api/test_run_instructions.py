...
def instructions_client(monkeypatch):
    token = "instruction-test-token-" + "x" * 43
    monkeypatch.setenv("GEIST_OPERATOR_TOKEN", token)
    monkeypatch.delenv("GEIST_OPERATOR_TOKEN_FILE", raising=False)
    monkeypatch.setattr("app.security.operator.get_default_workspace", lambda: SimpleNamespace(workspace_id=1))
    controls = RunControlRegistry()
    saved = []
    controls.start("owned", workspace_id=1, chat_id=7, on_instruction=saved.append)
    controls.start("other", workspace_id=2, chat_id=8)
    ...
    return TestClient(main.create_app(), headers={"Authorization": f"GeistOperator {token}"}), controls, saved
...
@pytest.mark.parametrize("text", ["", " ", "x" * 20_001], ids=["empty", "blank", "too-long"])
