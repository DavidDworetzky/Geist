...
class WorkspaceFileAdapter:
    ...
    def list_files(
        ...
        for root, directories, filenames in os.walk(search_root, followlinks=False):
            directories[:] = sorted(
                ...
                and not self._is_sensitive(directory)
    ...
    def _resolve(
        ...
        *,
        ...
        if self._is_sensitive(relative) or any(
            part.lower() in DEFAULT_IGNORED_DIRECTORIES for part in Path(relative).parts
        ):
    ...
    def _is_sensitive(path: str) -> bool:
        for part in Path(path).parts:
            ...
            if lowered in SENSITIVE_FILENAMES or lowered.startswith(".env."):
                return True
            if fnmatch.fnmatch(lowered, "service-account*.json"):
                return True
