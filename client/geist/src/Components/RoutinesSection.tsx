...
const RoutinesSection: React.FC = () => {
  ...
  const fetchRoutines = useCallback(async (quiet = false): Promise<void> => {
    ...
    try {
      ...
      if (!quiet) setError(err instanceof Error ? err.message : 'Failed to load routines');
    } finally {
      refreshing.current = false;
      setLoading(false);
      if (refreshPending.current) void fetchRoutines(true);
    }
  }, []);
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(() => {
      if (!document.hidden) void fetchRoutines(true);
    }, 15000);
    return () => window.clearInterval(timer);
  }, [fetchRoutines]);

  const createRoutine = async () => {
    if (!name.trim() || !prompt.trim() || !Number.isInteger(intervalMinutes)
      || intervalMinutes < 5 || intervalMinutes > 10080) return;
    try {
      setSaving(true);
      setError(null);
      const response = await fetch('/api/v1/routines/', {
  ...
  return (
    <section className="settings-section">
      ...
      {routines.some(routine => routine.scheduler_blocked) && (
        <div role="alert">The scheduler is waiting for a stopped worker to exit. Further routines are paused.</div>
      )}
      ...
      <div className="settings-subsection">
        ...
        <button
          ...
          disabled={saving || !name.trim() || !prompt.trim() || !Number.isInteger(intervalMinutes)
            || intervalMinutes < 5 || intervalMinutes > 10080}
        >
        ...
        {loading ? (
          ...
          routines.map((routine) => (
            ...
            >
              ...
              {routine.next_run_at && (
                <p className="settings-description">Next run: {new Date(routine.next_run_at).toLocaleString()}</p>
              )}
              {routine.run_once_requested && <p className="settings-description">Queued for the next scheduler check.</p>}
              {routine.last_status && <p className="settings-description">Last run: {routine.last_status}</p>}
              {routine.last_error && <p className="settings-description">{routine.last_error}</p>}
