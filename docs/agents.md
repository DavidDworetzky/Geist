...
- `GEMINI_API_KEY` - Google Gemini API key for Gemini models and image generation. Gemini 3.8 Flash ignores configured `n`, `temperature`, and `top_p` values because Google requires clients to omit those parameters.
