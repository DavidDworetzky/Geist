...
class Workflow(Base):
    ...
    workflow_id: Mapped[int] = Column(Integer, primary_key=True, autoincrement=True)
...
class WorkflowStep(Base):
    ...
    step_id: Mapped[int] = Column(Integer, primary_key=True, autoincrement=True)
...
class WorkflowRun(Base):
    ...
    run_id: Mapped[int] = Column(Integer, primary_key=True, autoincrement=True)
    workflow_id = Column(Integer, ForeignKey("workflow.workflow_id"))
    user_id = Column(Integer, ForeignKey("geist_user.user_id"))
...
class WorkflowStepResult(Base):
    ...
    result_id: Mapped[int] = Column(Integer, primary_key=True, autoincrement=True)
    run_id = Column(Integer, ForeignKey("workflow_run.run_id"))
    step_id: Mapped[int] = Column(Integer, ForeignKey("workflow_step.step_id"))
    status = Column(String, default=WorkflowRunStatus.RUNNING.value)
    started_at = Column(DateTime, default=datetime.datetime.utcnow)
    completed_at = Column(DateTime)
    input_data = Column(JSON)
...
def update_workflow(workflow_id: int, workflow_data: 'WorkflowUpdate') -> Workflow | None:
...
def create_workflow_run(workflow_id: int, user_id: int, input_data: dict[str, Any] | None = None) -> WorkflowRun:
...
def update_workflow_run_status(run_id: int, status: WorkflowRunStatus, error_message: str = None, output_data: dict[str, Any] = None) -> WorkflowRun | None:
...
def update_workflow_step_result(result_id: int, status: WorkflowRunStatus, output_data: dict[str, Any] = None, error_message: str = None) -> WorkflowStepResult | None:
