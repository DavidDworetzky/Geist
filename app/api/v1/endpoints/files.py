    ...
    try:
        ...
        if search:
            ...
            files = [
                f
                for f in files
                if search_lower in f.filename.lower() or search_lower in f.original_filename.lower()
            ]
...
async def get_file_metadata(file_id: int, current_user=Depends(get_current_user)) -> dict[str, Any]:
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to get file metadata: {str(e)}") from e
...
async def download_file(file_id: int, current_user=Depends(get_current_user)):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"File download failed: {str(e)}") from e
...
async def get_file_content(file_id: int, current_user=Depends(get_current_user)) -> dict[str, Any]:
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to get file content: {str(e)}") from e
...
async def delete_file(file_id: int, current_user=Depends(get_current_user)) -> dict[str, Any]:
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"File deletion failed: {str(e)}") from e
...
async def reprocess_file(
    ...
    try:
        ...
        return {"success": True, "file_id": file_id, "processing_result": processing_result}

    except HTTPException:
        raise
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"File reprocessing failed: {str(e)}") from e
...
async def get_processing_status(
    ...
    try:
        ...
        return {"success": True, **status_result}
