...
## planning
Create a plan in `/plans` only when a change touches backend architecture, such as core contracts, data models, service boundaries, API design, or agent and runner architecture.
Do not create plan or specification files for frontend-only changes, routine features, bug fixes, documentation, tests, or configuration changes. Record their scope and verification in the pull request description instead.

## implementation and validation
Implement backend architecture plans across the relevant data models, middle data models, service layer, routes, and backend tests.
