...
Resolve host workspace authorization before constructing execution environments;
disable misconfigured coding tools without preventing app startup. Protect shared
sandbox work with the destructive-command floor. Omitted API mode inherits the
saved preference; scheduled routines explicitly retain direct mode. Add an explicit
new-goal request/UI action, checkpoint recovery errors, typed duplicate-run errors,
and instruction events/history independent of goal state. Context compaction and
observation/instruction retention are intentionally deferred from this pass.
